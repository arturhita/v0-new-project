-- =================================================================
-- DEFINITIVE DROP AND RECREATE SCRIPT
-- This script is designed to be run on a potentially broken schema.
-- It corrects RLS policies, function recreation errors, and ensures
-- the entire financial and administrative backend schema is stable.
-- =================================================================
...
-- Drop constraint if it exists to avoid errors, then add it.
ALTER TABLE public.payout_requests
...
VALUES
    ('default_commission_rate', '30.00', 'Tasso di commissione predefinito per i nuovi operatori (es. 30.00 per 30%).', 'number'),
...
DROP POLICY IF EXISTS "Operators can create and view their own requests" ON public.commission_increase_requests;
CREATE POLICY "Operators can create and view their own requests" ON public.commission_increase_requests
...
CREATE POLICY "Admins can manage all payout requests" ON public.payout_requests
    FOR ALL USING (is_admin(auth.uid()));
DROP POLICY IF EXISTS "Operators can create and view their own payout requests" ON public.payout_requests;
CREATE POLICY "Operators can create and view their own payout requests" ON public.payout_requests
    FOR ALL USING (operator_id = auth.uid());

-- Step 5: Drop and Recreate RPC Functions

-- CORRECTED: Drop the function first to allow signature changes.
...
DROP FUNCTION IF EXISTS public.send_broadcast_notification(text, text, text);
