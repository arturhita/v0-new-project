...
-- Risolve i problemi di sintassi delle POLICY, RLS, tabelle mancanti e colonne mancanti.
...
-- Step 4: Elimina le policy esistenti (se ci sono) e le ricrea con la sintassi corretta.
...
DROP POLICY IF EXISTS "Operators can view their own commission requests" ON public.commission_increase_requests;
CREATE POLICY "Operators can view their own commission requests" ON public.commission_increase_requests
    FOR SELECT
    USING (auth.uid() = operator_id);

DROP POLICY IF EXISTS "Operators can create commission requests" ON public.commission_increase_requests;
CREATE POLICY "Operators can create commission requests" ON public.commission_increase_requests
    FOR INSERT
