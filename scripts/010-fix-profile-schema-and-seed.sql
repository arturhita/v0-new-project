...
-- Corregge lo schema della tabella 'profiles' e popola il database.
...
-- Rimuove la vecchia colonna 'specializations' se esiste, poiché ora usiamo una tabella di collegamento.
DO $$
BEGIN
...
-- Rimuove le tabelle esistenti per garantire una ricostruzione pulita.
...
COMMENT ON TABLE public.categories IS 'Memorizza le categorie di consulenza (es. Tarocchi, Astrologia).';

-- Crea la tabella di collegamento 'operator_categories'.
...
-- == SEZIONE 4: Popolamento del Database (Seeding) ==
...
-- Inserisce utenti e profili di esempio (clienti e operatori).
DO $$
DECLARE
  client_1_id UUID;
  client_2_id UUID;
