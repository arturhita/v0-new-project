...
export async function createInvoice(formData: FormData) {
  ...
  const rawFormData = {
    ...
    recipientType: formData.get('recipientType') as string, // 'client' or 'operator'
  };
  ...
  const invoiceData = {
    ...
    invoice_number: `INV-${Date.now()}`, // Simple unique number
    amount: parseFloat(rawFormData.amount),
