...
function OperatorDashboardLayoutWrapper({ children }: { children: React.ReactNode }) {
  const { user, loading } = useAuth()
  const router = useRouter()

  useEffect(() => {
    if (!loading && (!user || user.profile.role !== "operator")) {
      router.push("/login")
    }
  }, [user, loading, router])
...
export default OperatorDashboardLayoutWrapper
