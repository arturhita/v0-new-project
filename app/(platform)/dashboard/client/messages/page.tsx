...
import { getConversationsForClient, sendMessage } from "@/lib/actions/chat.actions"
  ...
  useEffect(() => {
    if (user) {
      const fetchConversations = async () => {
        setLoading(true)
        const result = await getConversationsForClient(user.id)
            ...
            conversations.map((convo) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage src={convo.operator.avatar_url || undefined} />
                    <AvatarFallback>{convo.operator.full_name?.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 overflow-hidden">
                    <p className="font-semibold truncate">{convo.operator.full_name}</p>
        ...
        {selectedConversation ? (
          <>
            <header className="p-4 border-b flex items-center gap-3">
              <Avatar>
                <AvatarImage src={selectedConversation.operator.avatar_url || undefined} />
                <AvatarFallback>{selectedConversation.operator.full_name?.charAt(0).toUpperCase()}</AvatarFallback>
              </Avatar>
              <h3 className="text-lg font-semibold">{selectedConversation.operator.full_name}</h3>
            </header>
