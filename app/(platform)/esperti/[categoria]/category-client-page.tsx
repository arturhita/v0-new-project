...
export default function CategoryClientPage({ initialOperators, currentUser, params }: CategoryClientPageProps) {
  ...
  const filteredOperators = useMemo(() => {
    ...
    // Filtra per termine di ricerca (nome o specializzazione)
    if (searchTerm) {
    ...
    if (priceSort !== "default") {
      const getPrice = (op: Operator) => {
        const priceString = String(op.services.chatPrice || op.services.callPrice || "999")
    ...
    <div className="bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800 text-white min-h-screen pt-16">
      <div className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 pt-8 md:pt-16 pb-16 md:pb-24 relative z-10">
          ...
          <div className="mb-12 p-6 bg-blue-900/50 backdrop-blur-sm rounded-2xl border border-yellow-600/20 shadow-lg flex flex-col md:flex-row gap-4 items-center">
          ...
          {filteredOperators.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
              {filteredOperators.map((operator) => (
                <div key={operator.id}>
                  <OperatorCard operator={operator} currentUser={currentUser} />
                </div>
              ))}
            </div>
          ) : (
