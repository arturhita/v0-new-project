...
export function SiteNavbar() {
  ...
  const getDashboardLink = () => {
    if (!profile) return "/login" // Usa il profilo per determinare il ruolo
    switch (profile.role) {
      case "admin":
  ...
  const renderUserMenu = () => {
    if (loading) {
      return <div className="h-10 w-28 animate-pulse rounded-md bg-white/10" />
    }
  ...
  const renderMobileAuthButtons = () => {
    if (loading) {
      return <div className="h-10 w-full animate-pulse rounded-md bg-white/10" />
    }
    if (!isAuthenticated || !user || !profile) {
      return (
        <>
    ...
    return (
      <>
        <Button variant="outline" className="w-full justify-center text-white border-blue-600 bg-blue-700" asChild>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[#1E3C98]/95 backdrop-blur-lg pb-8">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col space-y-4">
            <NavigationMenuDemo />
            <div className="flex flex-col space-y-2 pt-4 border-t border-blue-700">{renderMobileAuthButtons()}</div>
          </div>
        </div>
      )}
    </header>
