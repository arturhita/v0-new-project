    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      switch (paymentIntent?.status) {
        ...
        case "requires_payment_method":
          // Questo caso è gestito dal form, non dovrebbe apparire qui
          break
        default:
  ...
  if (isSucceeded) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-4">
        ...
        <h3 className="text-2xl font-bold text-white">Pagamento Riuscito!</h3>
        <p className="text-slate-300 mt-2">La tua consulenza è confermata.</p>
        <Button asChild className="mt-8 bg-yellow-400 text-slate-900 hover:bg-yellow-500 w-full">
          <Link href="/dashboard/client/consultations">Vai ai Tuoi Consulti</Link>
        </Button>
      </div>
    )
  }
